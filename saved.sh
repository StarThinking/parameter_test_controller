...
for i in $(grep -oP "node-[0-9]{1,2}$" /etc/hosts | sed 's/node-//g' | sort -n); do sleep 2; ssh node-$i "rm ~/nohup.txt; nohup ~/parameter_test_controller/dispatcher_hypo.sh> nohup.txt &" & pids[$i]=$!; done; for p in ${pids[@]}; do wait $p; echo "$p is done"; done
