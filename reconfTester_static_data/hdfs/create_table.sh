...
component=JournalNode; for i in org.*; do instances=$(java -cp ~/parameter_test_controller/reconfTester_static_data/hdfs/ InstanceAnalysis $i $component); if [ $instances -ge 2 ]; then parameter=$(echo $i | awk -F '-output.txt' '{print $1}'); echo $parameter' '$instances; fi; done > "$component"_Instance_GE_2.txt
